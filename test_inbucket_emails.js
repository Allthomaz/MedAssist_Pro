...
async function testEmailRecovery() {
  ...
  try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (emails.length > 0) {
            ...
            try {
              const emailResponse = await fetch(`${inbucketUrl}/api/v1/mailbox/${encodeURIComponent(mailboxName)}/${latestEmail.id}`);
              if (emailResponse.ok) {
                const emailContent = await emailResponse.json();
                console.log('📄 Conteúdo do email:');
                console.log('   - HTML:', emailContent.body.html ? 'Presente' : 'Ausente');
                console.log('   - Texto:', emailContent.body.text ? 'Presente' : 'Ausente');
              ...
              console.log('⚠️  Não foi possível buscar conteúdo do email:', error.message);
            }
    ...
    console.log('   - Configuração de naming do Inbucket diferente do esperado');
