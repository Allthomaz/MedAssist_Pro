...
async function testSupabaseEmailFlow() {
  try {
    ...
    const initialResponse = await fetch('http://127.0.0.1:54324/api/v1/messages');
    if (initialResponse.ok) {
      const initialMessages = await initialResponse.json();
      console.log(`📧 Emails existentes: ${initialMessages.messages ? initialMessages.messages.length : 0}`);
    }
    ...
    if (finalResponse.ok) {
      ...
      const confirmationEmail = messages.find(msg => 
        ...
        (msg.Subject.includes('Confirm') || msg.Subject.includes('confirm') || 
         msg.Subject.includes('Verificar') || msg.Subject.includes('verificar'))
      );
      ...
      if (confirmationEmail) {
        ...
        console.log('📧 Para:', confirmationEmail.To.map(to => to.Address).join(', '));
        console.log('📧 Data:', confirmationEmail.Created);
        
        // Tentar obter o conteúdo do email
        try {
          const emailContentResponse = await fetch(`http://127.0.0.1:54324/api/v1/message/${confirmationEmail.ID}`);
          if (emailContentResponse.ok) {
            ...
            const confirmLinkMatch = htmlContent.match(/http[s]?:\/\/[^\s<>"]+/g);
            if (confirmLinkMatch) {
              console.log('\n🔗 Links encontrados:');
        ...
        messages.slice(0, 5).forEach((msg, index) => {
          console.log(`   ${index + 1}. ${msg.Subject} (para: ${msg.To.map(to => to.Address).join(', ')})`);
        });
      }
    }
    ...
    if (data.user?.id) {
      ...
      console.log('ℹ️ Em ambiente de desenvolvimento, o usuário permanecerá no banco.');
    }
