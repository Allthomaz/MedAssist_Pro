  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-grid">
        ...
        <ConsultationTrendsChart
          data={dashboardData?.charts.consultationTrends}
        />
...
```tsx
const QuickStatsCards: React.FC<{ metrics: OverviewMetrics }> = ({
  metrics,
}) => {
  const stats = [
    {
...
```tsx
const PatientGrowthChart: React.FC<{ data: PatientGrowthData[] }> = ({
  data,
}) => {
  const [selectedMetric, setSelectedMetric] = useState<
    'newPatients' | 'totalPatients' | 'growthRate'
  >('newPatients');
...
```tsx
const ConsultationTrendsChart: React.FC<{ data: ConsultationTrendData[] }> = ({
  data,
}) => {
...
```tsx
const RecentActivityFeed: React.FC<{ activities: ActivityItem[] }> = ({
  activities,
}) => {
  ...
  const activityTypes = [
    ...
    {
      value: 'transcription_finished',
      label: 'Transcrição Finalizada',
      icon: FileText,
      color: 'purple',
    },
    {
  ...
  return (
    <Card className="activity-feed">
      ...
      <CardContent>
        <div
          className={`activity-list ${isExpanded ? 'expanded' : 'collapsed'}`}
        >
                      ...
                      <div className="activity-actions">
                        {activity.metadata.patientId && (
                          <Button
                            ...
                            size="xs"
                            onClick={() =>
                              navigateToPatient(activity.metadata.patientId)
                            }
                          >
                            Ver Paciente
                          </Button>
...
class DashboardService {
   ...
   */
  private async loadOverviewMetrics(
    dateRange: DateRange,
    doctorId: string
  ...
  private async exportToPDF(data: DashboardData): Promise<Blob> {
    ...
    doc.setFontSize(12);
    doc.text(
      `Período: ${formatDate(data.dateRange.start)} - ${formatDate(data.dateRange.end)}`,
      20,
      45
    );
  ...
  private async exportToExcel(data: DashboardData): Promise<Blob> {
    ...
    const metricsData = [
      ...
      ['Consultas (mês)', data.overview.consultationsThisMonth],
      [
        'Taxa de Crescimento Consultas (%)',
        data.overview.consultationGrowthRate,
      ],
