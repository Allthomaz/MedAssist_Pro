    ...
    <div className="settings-layout">
      ...
      <div className="settings-content">
        ...
        <div className="settings-main">
          ...
          <div className="settings-content-area">{children}</div>
        </div>
      </div>
    </div>
  );
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Hospital ou clínica onde atua"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
      ...
      if (categories && categories.length > 0) {
        ...
        categories.forEach(category => {
          if (defaultSettings[category as keyof UserSettings]) {
            (resetSettings as any)[category] =
              defaultSettings[category as keyof UserSettings];
          }
        });
...
const checkPermission = (
  ...
  if (resource === 'own_settings' && user.role === 'patient') {
    return (
      userPermissions.includes('read_own') && ['read', 'write'].includes(action)
    );
  }
...
export const professionalInfoSchema = z.object({
  ...
  crm: z
    .string()
    .regex(/^\d{4,6}\/[A-Z]{2}$/, 'Formato inválido. Use: 123456/UF'),
