  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const { count: transcriptionsCount } = await supabase
        .from('transcriptions')
        .select('*, recordings!inner(doctor_id)', {
          count: 'exact',
          head: true,
        })
  ...
  return (
    <MedicalLayout>
      <div className="flex flex-col gap-6">
        {/* Header */}
        <div
          className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 medical-header ${compactMode ? 'medical-spacing' : 'medical-spacing-lg'}`}
        >
          <div className="space-y-1">
            <h1
              className={`font-bold text-foreground tracking-tight ${compactMode ? 'medical-text-2xl' : 'text-3xl'}`}
            >
              Dashboard
            </h1>
            <p
              className={`text-muted-foreground ${compactMode ? 'text-sm' : 'text-base'}`}
            >
              Bem-vindo de volta, {fullDisplayName}.
            </p>
            <p
              className={`text-muted-foreground ${compactMode ? 'text-xs' : 'text-sm'}`}
            >
              {professionName}
            </p>
            {profile?.custom_title && (
              <div
                className={`custom-title-highlight mt-2 ${compactMode ? 'medical-spacing' : ''}`}
              >
                <p
                  className={`text-medical-blue font-medium ${compactMode ? 'text-xs' : 'text-sm'} flex items-center gap-2`}
                >
        ...
        {/* Stats Cards */}
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 ${compactMode ? 'medical-spacing' : ''}`}
        >
        ...
        {/* Main Content Grid */}
        <div
          className={`grid grid-cols-1 lg:grid-cols-3 gap-6 items-start ${compactMode ? 'medical-spacing' : ''}`}
        >
          {/* Quick Actions */}
        ...
        <div
          className={`grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6 ${compactMode ? 'medical-spacing' : ''}`}
        >
          <Card
            className={`medical-card medical-card-hover ${compactMode ? 'medical-card' : ''}`}
          >
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle
                className={`font-semibold text-gray-900 ${compactMode ? 'text-base' : 'text-lg'}`}
              >
                Pacientes Recentes
              </CardTitle>
              <Button
