  ...
  try {
    ...
    try {
      const statusResponse = await fetch(
        'http://127.0.0.1:54324/api/v1/monitor/messages'
      );
    ...
    try {
      const mailboxResponse = await fetch(
        'http://127.0.0.1:54324/api/v1/mailbox'
      );
          ...
          for (const mailbox of mailboxData) {
            ...
            try {
              const emailResponse = await fetch(
                `http://127.0.0.1:54324/api/v1/mailbox/${encodeURIComponent(mailbox)}`
              );
              if (emailResponse.ok) {
                ...
                emails.forEach((email, index) => {
                  console.log(
                    `   - Email ${index + 1}: ${email.subject} (de: ${email.from})`
                  );
                });
              } else {
